...
import { ShoppingCart, Utensils, Search, Plus, Minus, X, CheckCircle, Clock, Wifi, WifiOff } from 'lucide-react';
...
export default function OrderPage({ searchParams }: { searchParams: { restaurantId: string; tableId: string } }) {
  ...
  const [mqttStatus, setMqttStatus] = useState<'disconnected' | 'connecting' | 'connected'>('connecting');
  ...
  const placeOrder = async () => {
    try {
      ...
      // 2. Publish to MQTT for Real-time Kitchen Notification
      if (mqttClient && mqttStatus === 'connected') {
        const topic = `restaurant/snmimt/table/${tableId || 'T01'}`;
        ...
        mqttClient.publish(topic, payload, { qos: 1 }, (err?: Error) => {
          ...
          else console.log('🚀 Order published to MQTT Cloud Topic:', topic);
        });
      }
      ...
      <header className="sticky top-0 bg-white shadow-sm z-50 px-6 py-4 flex justify-between items-center border-b border-indigo-100">
        <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-1.5 ml-2">
                ...
                <span className={`text-[8px] font-black uppercase tracking-tighter ${mqttStatus === 'connected' ? 'text-emerald-500' : 'text-rose-400'}`}>
                    {mqttStatus === 'connected' ? 'Live' : 'Retrying...'}
                </span>
            </div>
        </div>
